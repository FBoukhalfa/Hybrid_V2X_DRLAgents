...
lr_actor_list =  [1e-2, 1e-3]       # learning rate for actor network ==> 0.0003 ==> pdr = 94.000%
lr_critic_list = [1e-2,1e-3]
eps_clip_list =  [0.01,0.1]
...
for lr_actor in lr_actor_list:
    for lr_critic in lr_critic_list:
        for eps_clip in eps_clip_list:
                ...
                command = f"python3.8 ../Training_and_Test_scripts/train.py --lr_actor {lr_actor} --lr_critic {lr_critic} --eps_clip {eps_clip} --namefile {namefile}" 
